...
- **Пиксельный экран.** `esp.sendFrame(buffer, w, h, bpp)` отправляет настоящие
  пиксели — картинки, фото, графику, как на Flipper Zero / spacedesk (см. пример `05_PixelScreen`);
- **Виртуальный экран (текст).** Всё, что вы `print()`ите из скетча, приложение
  показывает на своём экране;
...
| `COL <r> <g> <b> <имя>`        |                          |
| `PIX <w> <h> <bpp> <len> <base64>` |                       |

Цвет экрана из скетча — `esp.setColor(r, g, b)`: приложение перекрашивает
виртуальный экран (см. пример `03_ColorScreen`).

## Пиксельный экран (пример 05)

```cpp
static uint8_t frame[128 * 64 / 8];          // буфер монохромного экрана

void loop() {
  esp.loop();
  drawSomethingInto(frame);                  // ваше рисование
  esp.sendFrame(frame, 128, 64, 1);          // bpp: 1 | 8 | 16 (RGB565) | 24 (RGB888)
  delay(100);
}
```

Кадр уходит как `PIX <w> <h> <bpp> <len> <base64>` и кэшируется библиотекой:
при подключении нового клиента или `PING` он пересылается заново. По BLE
длинная строка автоматически режется на куски, укладывающиеся в MTU.
Монохром (bpp=1) упаковывается по 8 пикселей в байт, биты MSB-first.
